...
- 支持显式命令：
  ...
  - `/spamadd <关键词>`：添加广告关键词；
  - `/spamdel <关键词>`：删除广告关键词；
  - `/r`：默认的监控已读快捷命令，可在面板改成别的；会将当前管理员聊天中此前的监控通知标记为已读，并把命令消息和机器人确认一起加入延时删除队列；
  - `/cancel`：取消待发送图片。
- 普通用户有简单限流，防止刷屏。
...
- 支持去重，避免同一条反复推送。
- 支持在监控编辑页填写屏蔽关键词；标题、正文、链接、作者、分类、发布时间、价格、库存及最终通知文本命中任意屏蔽词时都不推送，匹配不区分大小写。
- TG 群监听的屏蔽词同样覆盖群名、发送者、消息链接、回复和正文；AI 总结会在生成后再检查一次。
- 被屏蔽条目仍会写入去重状态，之后移除屏蔽词也不会补推旧帖；作者、分类白名单过滤仍可通过 YAML 高级配置使用。
- 单个监控可关闭 Telegram 推送，只记录到 Web 推送历史。
- 默认最低监控间隔为 60 秒。
...
- 收件箱页面，可查看完整双向对话记录、消息类型、重试转发、直接回复。
...
- CMDE 官网通过项目内现有 FlareSolverr 渲染，使用列表链接中的精确发布时间进入 36 小时 IVD 过滤与独立推送流程，不依赖 RSSHub。
- 巨潮资讯通过官方公告 JSON 接口直接接入，不依赖 RSSHub；默认观察 14 家国内 IVD 上市公司，并复用相同的 36 小时过滤、去重和推送链路。
...
- 命中 `keywords` 且最终通知内容未命中 `exclude_keywords` 时，会给管理员发送摘要。
...
IDCFLARE 公开 RSS 为 `https://idcflare.com/latest.rss`。它对普通服务器请求启用了 Cloudflare 浏览器挑战，因此 Docker 部署会同时启动一个不暴露端口的 FlareSolverr 服务。程序只在 IDCFLARE 返回挑战页时才使用该服务，并会将浏览器 RSS 预览页中的 XML 还原后再解析。烧饼论坛可直接使用 `https://sb.sb/rss.xml`，无需浏览器后备。Linux.SB 没有公开 RSS，内置模板通过按发帖时间排序的 `https://linux.sb/index.php?sort=post` 监控新帖，避免旧帖因新回复回到列表前部，并在首次启用时只建立基线。详见 [RSS 监控](docs/rss-monitoring.md)。
...
说明：`/r` 是默认的监控已读快捷命令，可在面板里改成任意其他命令；旧的 `/monread` 也兼容可用。
